...
async function getChangelogMessage() {
  ...
  return new Promise((resolve, reject) => {
    let changelog = "";
    const pkg = packages.find((p) => p.name === "@mvidalgarcia/a");
    const stream = conventionalChangelog(
      ...
      },
      {
        ...
        title: "@mvidalgarcia/a",
        owner: "mvidalgarcia",
        repository: "dummy-monorepo",
        linkCompare: true,
        version: pkg.version,
      }
    );
...
(async () => {
  try {
    ...
    if (argv.dry) {
      ...
      await $`yarn install`;
      await $`yarn lerna publish --no-private --conventional-commits --create-release github --yes`;
